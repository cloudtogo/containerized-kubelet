...
const SkippedVersionLabelPrefix = "skip-";
        ...
        for (const issue of issues) {
            if (issue.labels.map(l => l.name).includes(ReleaseLabel)) {
                ...
                issue.labels.reduce((versions, l) => {
                    ...
                    var label = l.name;
                    if (l.name.startsWith(SkippedVersionLabelPrefix)) {
                        label = l.name.slice(SkippedVersionLabelPrefix.length);
                    }

                    const v = parseVersionFromLabel(label);
...
module.exports.checkUpstreamRelease = async (github) => {
  ...
  for (const rel of ltsReleases) {
      ...
      if (imageVersions[rel].minor < releases[rel].minor) {
          ...
          skippedReleases.push(SkippedVersionLabelPrefix + imageVersions[rel].version);
      }
  }
  ...
  await github.issues.create({
    ...
    labels: newReleases.concat(skippedReleases),
...
module.exports.readKubeVersionFromLabels = () => {
  var kubeVersions = process.env.ISSUE_LABELS.split(" ").reduce((versions, l) => {
      if (l == ReleaseLabel) {
          return versions;
      }

      if (l.startsWith(SkippedVersionLabelPrefix)) {
          versions.push(l.slice(SkippedVersionLabelPrefix.length));
      } else {
          versions.push(l);
      }
